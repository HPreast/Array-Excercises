...
export const getStudentByName = (fullName) => {
    return students.find(student => {
        return (`${student.firstName.toLowerCase()} ${student.lastName.toLowerCase()}` === fullName.toLowerCase());
    })
}
...
export const getStudentWithMostLangs = () => {
    let knownLanguages = students.map(student => student.languages.length);
    let mostKnown = Math.max(...knownLanguages)

    return students.find(student => student.languages.length === mostKnown);
}