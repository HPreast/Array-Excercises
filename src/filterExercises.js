...
export const getStudentsInCohort = (cohort) => {
    const studentsInCohort = students.filter(students => students.cohort === cohort);
    console.log("return", studentsInCohort);
...
export const getFullTimeStudents = () => {
    const fullTimeStudents = students.filter(students => students.fullTime === true);
...
export const getStudentsByInstructorId = (instructorId) => {
    const studentsByInstructor = students.filter(students => students.instructorId === instructorId);
...
export const getPolyglotStudents = (languageCount) => {
    const polyglotStudents = students.filter(students => students.languages.length >= languageCount);
...
export const getAvailableInstructors = () => {
    let studentArray = students.map(student => student.instructorId)
    return instructors.filter(instructor => {
        if (studentArray.includes(instructor.id) === false) {
...
export const getStudentsByLanguage = (language) => {
    const knowLanguage = students.filter(student => student.languages.includes(language))
    console.log("language", knowLanguage)
    return knowLanguage;
}
/******** ADVANCED CHALLENGE ********/
...
export const getStudentsByLanguages = (languages) => {
    return students.filter(student => {
        let knownLanguages = languages.filter(language => {
            let lang = student.languages.find(studentLang => studentLang === language)
            return lang
        })
